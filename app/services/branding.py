"""
Branding service — color palette generation and logo color extraction.

The palette is a dict of Tailwind-compatible shade keys (50–950) for two
colors: ``navy`` (primary) and ``accent``.  The shades are generated by
fixing the hue/saturation of the base color and interpolating lightness
across the Tailwind scale.

The palette dict is injected into every template via the context processor
in ``app/__init__.py`` and consumed by the Tailwind config in ``base.html``.
"""
